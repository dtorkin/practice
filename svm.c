/*
svm.c:
•	Логика таймеров и счётчиков
•	Реализация обработчиков сообщений, приходящих от УВМ (handle_***_message)
•	Функция main:
	o	Установка сетевого соединения с УВМ
	o	Запуск таймеров и счётчиков
	o	Инициализация диспетчера
	o	Главный цикл: прием сообщений (заголовок + тело), вызов обработчика через диспетчер
...
void handle_prinyat_time_ref_range_message(int clientSocketFD, Message *receivedMessage); 		// 4.2.10. «Принять TIME_REF_RANGE» 4.2.10 (заглушка)
void handle_prinyat_reper_message(int clientSocketFD, Message *receivedMessage);		  		// 4.2.11. «Принять Reper» (заглушка)
...
void handle_prinyat_ref_azimuth_message(int clientSocketFD, Message *receivedMessage);   		// 4.2.14. «Принять REF_AZIMUTH» (заглушка)
...
// [4.2.10] «Принять TIME_REF_RANGE» (обработчик сообщений) - заглушка
...
// [4.2.11] «Принять Reper» (обработчик сообщений) - заглушка
void handle_prinyat_reper_message(int clientSocketFD, Message *receivedMessage) {
	...
	// Пример вывода данных первой точки
...
// [4.2.14] «Принять REF_AZIMUTH» (обработчик сообщений) - ИСПРАВЛЕННАЯ ВЕРСИЯ
void handle_prinyat_ref_azimuth_message(int clientSocketFD, Message *receivedMessage) {
	...
	// Проверяем ожидаемый размер тела (2 байта NTSO + 32768 байт массив)
	if (receivedMessage->header.body_length != sizeof(PrinyatRefAzimuthBody)) {
		...
		return; // Прекращаем обработку, если размер неверный
	}

	// Кастуем тело к правильной, исправленной структуре
	PrinyatRefAzimuthBody *body = (PrinyatRefAzimuthBody*)receivedMessage->body;

	// Выводим номер цикла (он уже преобразован в message_to_host_byte_order)
	printf("  NTSO: %u\n", body->NTSO);

	// Выводим первый элемент массива для примера
	// (он уже преобразован в message_to_host_byte_order)
	...
	// memcpy(svm_internal_ref_azimuth_buffer, body->ref_azimuth, sizeof(body->ref_azimuth));
}
...
void init_message_handlers() {
	...
	message_handlers[MESSAGE_TYPE_INIT_CHANNEL]			 = handle_init_channel_message;					// 4.2.1. «Инициализация канала»
	message_handlers[MESSAGE_TYPE_CONFIRM_INIT]			 = handle_confirm_init_message;					// 4.2.2. «Подтверждение инициализации канала»
	message_handlers[MESSAGE_TYPE_PROVESTI_KONTROL]		 = handle_provesti_kontrol_message;				// 4.2.3. «Провести контроль»
	message_handlers[MESSAGE_TYPE_PODTVERZHDENIE_KONTROLYA] = handle_podtverzhdenie_kontrolya_message;	// 4.2.4. «Подтверждение контроля»
	message_handlers[MESSAGE_TYPE_VYDAT_RESULTATY_KONTROLYA] = handle_vydat_rezultaty_kontrolya_message;// 4.2.5. «Выдать результаты контроля»
	message_handlers[MESSAGE_TYPE_RESULTATY_KONTROLYA]	  = handle_rezultaty_kontrolya_message;			// 4.2.6. «Результаты контроля»
	message_handlers[MESSAGE_TYPE_VYDAT_SOSTOYANIE_LINII]	= handle_vydat_sostoyanie_linii_message;	// 4.2.7. «Выдать состояние линии»
	message_handlers[MESSAGE_TYPE_SOSTOYANIE_LINII]		  = handle_sostoyanie_linii_message;			// 4.2.8. «Состояние линии»
	message_handlers[MESSAGE_TYPE_PRIYAT_PARAMETRY_SO]	  = handle_prinyat_parametry_so_message;		// 4.2.9. «Принять параметры СО»
	message_handlers[MESSAGE_TYPE_PRIYAT_TIME_REF_RANGE]	= handle_prinyat_time_ref_range_message;	// 4.2.10. «Принять TIME_REF_RANGE»
	message_handlers[MESSAGE_TYPE_PRIYAT_REPER]			 = handle_prinyat_reper_message;				// 4.2.11. «Принять Reper»
	message_handlers[MESSAGE_TYPE_PRIYAT_PARAMETRY_SDR]	 = handle_prinyat_parametry_sdr_message;		// 4.2.12. «Принять параметры СДР»
	message_handlers[MESSAGE_TYPE_PRIYAT_PARAMETRY_3TSO]	= handle_prinyat_parametry_3tso_message; 	// 4.2.13. «Принять параметры 3ЦО»
	message_handlers[MESSAGE_TYPE_PRIYAT_REF_AZIMUTH]	   = handle_prinyat_ref_azimuth_message;		// 4.2.14. «Принять REF_AZIMUTH»
	message_handlers[MESSAGE_TYPE_PRIYAT_PARAMETRY_TSD]	= handle_prinyat_parametry_tsd_message;			// 4.2.15. «Принять параметры ЦДР»
	message_handlers[MESSAGE_TYPE_NAVIGATSIONNYE_DANNYE]	= handle_navigatsionnye_dannye_message;		// 4.2.16. «Навигационные данные»
}
